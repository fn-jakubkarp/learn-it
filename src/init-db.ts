...
// (see src/learn-it.ts): the path is file-relative, never the caller's cwd, so
// it can't strand a fresh empty db in the wrong directory. `dataDir` is a seam
// for tests only — production always uses the default. Returns the db path.
export function initDb(dataDir: string = DEFAULT_DATA_DIR): string {
	const dbPath = path.join(dataDir, "learn_it.db");
	...
	// Home assessments issued by `assess`: the queryable index of the markdown task
	// files under subjects/<s>/assessments/. A row is 'pending' from issue until
	// `evaluate` grades the matching file and flips it to 'done' (with the score),
	// so pending work nags on the dashboard instead of being silently forgotten.
	db.run(
...
// Standalone entrypoint: `bun src/init-db.ts`. When imported (by the CLI's
// first-run self-heal or by tests), `import.meta.main` is false, so nothing runs
// on import — only the exported `initDb` does.
if (import.meta.main) {
	console.log(`learn-it database ready at ${initDb()}`);
}
