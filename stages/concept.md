...
4. **Learner restates, in their words.** They explain it back and write that into `subjects/<subject>/notes.md` (if new, open it with frontmatter — `type: notes`, `subject: <subject>`). Producing the explanation *is* the encoding (the generation effect) — so the learner writes it; the agent only corrects. Never author the note for them.
