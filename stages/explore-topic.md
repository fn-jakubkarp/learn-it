...
4. Write the candidate list to `subjects/<subject>/roadmap.md` and register each — registration must happen here so `explore-gaps` has concepts to `probe` against:
   ```bash
